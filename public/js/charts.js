...
export function renderImpressions(canvas, monthlyRows, channel, { notes = [], goal } = {}) {
  ...
  if (channel === 'all' || channel === 'google')
    datasets.push({ label: 'Google', data: s.google, backgroundColor: 'rgba(66,133,244,0.7)', borderRadius: 3 });
  if (channel === 'all' || channel === 'meta')
    datasets.push({ label: 'Meta',   data: s.meta,   backgroundColor: 'rgba(8,102,255,0.5)',  borderRadius: 3 });
...
export function renderClicksCtr(canvas, monthlyRows, channel, { notes = [] } = {}) {
  ...
  charts.clicksCtr = new Chart(canvas, {
    data: {
      labels,
      datasets: [
        { type: 'bar',  label: 'Cliques', data: clicks, backgroundColor: 'rgba(0,173,167,0.55)', borderRadius: 2, yAxisID: 'y' },
