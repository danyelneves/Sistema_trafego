 ...
 *
 * Uso:  npm run backup
 * Destino: db/backups/maranet-YYYY-MM-DD-HHmm.db
...
const DB_FILE   = path.join(__dirname, 'maranet.db');
...
// Nome: maranet-2026-04-23-1430.db
const now = new Date();
const pad = n => String(n).padStart(2, '0');
const stamp = `${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())}-${pad(now.getHours())}${pad(now.getMinutes())}`;
const dest  = path.join(BACKUP_DIR, `maranet-${stamp}.db`);
...
const files = fs.readdirSync(BACKUP_DIR)
  .filter(f => f.startsWith('maranet-') && f.endsWith('.db'))
  .sort();
